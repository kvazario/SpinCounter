...
var render = function render(url) {
  ...
  path == '' ? $('.nav__link[href=\'#\']').addClass('nav__link--active') : $('.nav__link[href=\'' + path + '\']').addClass('nav__link--active');

  // go to
  if (map[path]) map[path]();else render404();
};
...
if (isOpera || isFirefox || isChrome) $('.wrong-browser').css('display', 'none');
...
var PSeu = function () {
  function PSeu() {
    ...
    this.chances = {
      '0.25': [0.494882, 0.414012, 0.085, 0.005, 0.001, 0.000075, 0.00003, 0.000001],
      '1': [0.494882, 0.414012, 0.085, 0.005, 0.001, 0.000075, 0.00003, 0.000001],
      '3': [0.494882, 0.414012, 0.085, 0.005, 0.001, 0.000075, 0.00003, 0.000001],
      '5': [0.734388, 0.184506, 0.075, 0.005, 0.001, 0.000075, 0.00003, 0.000001],
      '7': [0.464882, 0.444012, 0.085, 0.005, 0.001, 0.000075, 0.00003, 0.000001],
      '10': [0.719388, 0.199506, 0.075, 0.005, 0.001, 0.000075, 0.00003, 0.000001],
      '15': [0.434882, 0.474012, 0.085, 0.005, 0.001, 0.000075, 0.00003, 0.000001],
      '30': [0.434882, 0.474012, 0.085, 0.005, 0.001, 0.000075, 0.00003, 0.000001],
      '60': [0.404882, 0.504012, 0.085, 0.005, 0.001, 0.000075, 0.00003, 0.000001],
      '100': [0.404882, 0.504012, 0.085, 0.005, 0.001, 0.000075, 0.00003, 0.000001]
    };
  ...
  _createClass(PSeu, [{
    ...
    value: function parseAll(files) {
      ...
      var keys = Object.keys(files); // array of files in folder

      // loop through each file and call read function
      var progress = keys.map(function (item) {
        return new Promise(function (resolve) {
          _this.readFile(files[item], resolve);
        });
      });
      ...
      Promise.all(progress).then(function () {
        ...
        // all limits in room
        var knownLimits = Object.keys(_this.tournaments);

        // only limits that were played in import history
        var seenLimits = knownLimits.filter(function (x) {
          return _this.tournaments[x].length > 0;
        }).sort(function (a, b) {
          return a - b;
        });

        // minimal of seen limit
        var minLimit = Math.min.apply(Math, _toConsumableArray(seenLimits));
        ...
        if (seenLimits.length > 0) {
          ...
          _this.renderResults(minLimit, seenLimits);
        } else {
    ...
    key: 'readFile',
    value: function readFile(e, callback) {
      var that = this;
      ...
      if (e.name.length < 60) reader.readAsText(e, "UTF-8");else {
      ...
      reader.onload = function (evt) {
        ...
        try {
          var text = evt.target.result; // get the text of the file
          ...
          if ((rows[0].split(' ')[1] == 'Tournament' || rows[0].split(' ')[1] == 'Турнир') && rows[2].split(' ')[0] == '3') {
            ...
            if (rows[10].split(' ')[0] == 'You' && rows[10].split(' ')[1] == 'finished' || rows[10].split(' ')[0] == 'Вы' && rows[10].split(' ')[1] == 'финишировали') {
              ...
              var initialBuyin = rows[1].split(' ')[1];
              var buyin = +initialBuyin.split('/')[0].slice(1) + +initialBuyin.split('/')[1].slice(1); // e.g. 7

              var date = rows[4].split(' ')[2]; // e.g. '2016/11/08' or '17.12.2016'
              if (date.indexOf('.') != -1) date = date.split('.')[2] + '/' + date.split('.')[1] + '/' + date.split('.')[0];else date = date.split('/')[1] + '/' + date.split('/')[2] + '/' + date.split('/')[0];

              var prizePool = rows[3].split(' ')[3].slice(1);
              var multiplier = +prizePool / buyin;

              var won = rows[10].split(' ')[3] == '1st' || rows[10].split(' ')[3] == '1';
    ...
    key: 'renderResults',
    value: function renderResults(limit, seenLimits) {

      // clear limits buttons
      $('#limits').html('');

      // for each seen limit in import history create button with click event
      seenLimits.map(function (x) {
        var btn = '<li class="limits__item';
    ...
    key: 'draw',
    value: function draw(tournaments, limit) {
      var _this2 = this;
      ...
      if (limit != 'Total') {
        ...
        this.multipliers.map(function (item, indexOfMult) {

          // multiplier
          var mult = 'x' + item;

          // count = array filtered by multiplier
          var count = tournaments[limit].filter(function (x) {
            return x[1] == item;
          }).length;

          // won = array filtered by multiplier and won = true
          var won = tournaments[limit].filter(function (x) {
            return x[1] == item && x[2] == true;
          }).length;

          // itm = won / count in percentage, return 0 when divide by 0
          var itm = Math.round(won / count * 10000) / 100;
          if (isNaN(itm)) itm = 0;

          // expected count = chance * total count
          var exp = Math.round(that.chances[limit][indexOfMult] * tournaments[limit].length);
          ...
          var diffColor = diff < 0 ? 'results__cell--red' : diff > 0 ? 'results__cell--green' : '';

          // profit = prizes - buyins
          var profit = limit * (item * won - count);
          ...
          var profitColor = profit < 0 ? 'results__cell--red' : profit > 0 ? 'results__cell--green' : '';

          var row = '<tr class="results__row">\n                        <td class="results__cell">' + mult + '</td>\n                        <td class="results__cell">' + count + '</td>\n                        <td class="results__cell">' + exp + '</td>\n                        <td class="results__cell ' + diffColor + '">' + diff + '</td>\n                        <td class="results__cell">' + won + '</td>\n                        <td class="results__cell">' + itm + '</td>\n                        <td class="results__cell ' + profitColor + '">' + profit + '</td>\n                      </tr>';
          $('#results').append(row);
        });

        // get summary and draw summary row
        var sumCount = tournaments[limit].length;

        var sumWon = tournaments[limit].filter(function (x) {
          return x[2] == true;
        }).length;

        var sumITM = Math.round(sumWon / sumCount * 10000) / 100;
        if (isNaN(sumITM)) sumITM = 0;

        var sumExpWithJP = expectedArr.reduce(function (prev, cur) {
          return prev + cur;
        }, 0);
        var sumExpWithoutJP = sumExpWithJP - expectedArr[expectedArr.length - 1] - expectedArr[expectedArr.length - 2] - expectedArr[expectedArr.length - 3];
        sumExpWithJP = Math.round(sumExpWithJP);
        sumExpWithoutJP = Math.round(sumExpWithoutJP);

        var sumDiffWithJP = diffArr.reduce(function (prev, cur) {
          return prev + cur;
        }, 0);Math.round();
        var sumDiffWithoutJP = sumDiffWithJP - diffArr[diffArr.length - 1] - diffArr[diffArr.length - 2] - diffArr[diffArr.length - 3];
        ...
        // limits seen
        var lims = Object.keys(tournaments).filter(function (lim) {
          return tournaments[lim].length > 0;
        });
        ...
        this.multipliers.map(function (item, indexOfMult) {
          ...
          // count = array filtered by multiplier
          var count = lims.map(function (lim) {
            return tournaments[lim].filter(function (x) {
              return x[1] == item;
            }).length;
          ...
          // profit = prizes - buyins
          var profit = lims.map(function (lim) {
            return that.multipliers.indexOf(item) + 3 >= that.multipliers.length ? lim * ((that.jp - 2) / that.jp * item * tournaments[lim].filter(function (x) {
              return x[1] == item && x[2] == true;
            }).length - tournaments[lim].filter(function (x) {
              return x[1] == item;
            }).length + item / that.jp * (tournaments[lim].filter(function (x) {
              return x[1] == item;
            }).length - tournaments[lim].filter(function (x) {
              return x[1] == item && x[2] == true;
            }).length)) : lim * (item * tournaments[lim].filter(function (x) {
              return x[1] == item && x[2] == true;
            }).length - tournaments[lim].filter(function (x) {
              return x[1] == item;
            }).length);
        ...
        var _sumITM = Math.round(_sumWon / _sumCount * 10000) / 100;
        if (isNaN(_sumITM)) _sumITM = 0;

        var sumBuyin = lims.map(function (lim) {
          return tournaments[lim].length * lim;
        }).reduce(function (sum, next) {
          return sum + next;
        }, 0);

        var _ROI = Math.round(_sumProfit / sumBuyin * 10000) / 100;
        var _ROIColor = _ROI < 0 ? 'results__cell--red' : _ROI > 0 ? 'results__cell--green' : '';

        var _sumProfitColor = _sumProfit < 0 ? 'results__cell--red' : _sumProfit > 0 ? 'results__cell--green' : '';

        var _rake = Math.round(lims.map(function (lim) {
          return tournaments[lim].length * _this2.rake[lim];
        }).reduce(function (sum, next) {
          return sum + next;
        }, 0) * 100) / 100;

        var _vpp = Math.round(_rake * this.vpp * 100) / 100;

        var _summaries = '<tr class="results__row">\n                          <td class="results__cell results__cell--double">Summary</td>\n                          <td class="results__cell results__cell--double">' + _sumCount + '</td>\n                          <td class="results__cell results__cell--double">' + _sumWon + '</td>\n                          <td class="results__cell results__cell--double">' + _sumITM + '</td>\n                          <td class="results__cell results__cell--double ' + _sumProfitColor + '">' + _sumProfit + '</td>\n                        </tr>\n                        <tr class="results__row">\n                          <td class="results__cell ' + _ROIColor + '" colspan="2">% ROI = ' + _ROI + '</td>\n                          <td class="results__cell" colspan="2">Rake paid = $' + _rake + '</td>\n                          <td class="results__cell">VPP = ' + _vpp + '</td>\n                        </tr>';
    ...
    value: function filter() {
      ...
      var filtering = Object.keys(this.tournaments).map(function (lim) {
        that.tournaments[lim].map(function (tourn) {
          return new Promise(function (resolve) {
      ...
      // draw the results after read all files
      Promise.all(filtering).then(function () {

        // all limits in room
        var knownLimits = Object.keys(_this3.tournaments);

        // only limits that were played in filtered range
        var seenLimits = knownLimits.filter(function (x) {
          return _this3.filtered[x].length > 0;
        }).sort(function (a, b) {
          return a - b;
        });
        ...
        // if there is at least 1 valid tournament in import
        if (seenLimits.length > 0) {
          _this3.isFiltered = true;
          $('.error-result').addClass('hidden');
          _this3.renderResults(minLimit, seenLimits);
        } else {
          _this3.isFiltered = false;
...
var switchLimit = function switchLimit(e) {
  ...
  } else room.draw(room.tournaments, $(e).data('limit'));
};
